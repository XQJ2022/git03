...
void Uart0_Init(void)
{
	...
	UART0->UARTCR_b.UARTEN=1;     // UART 使能
	UART0->UARTIBRD=39;	      // 波特率分频比的整数部分
	UART0->UARTFBRD=4;	      // 波特率分频比的小数部分
...
//	UART0->UARTFBRD=48; 	// 对应9600bps波特率
	...
	UART0->UARTIMSC_b.Receive_IM =1;  // 关闭接收中断屏蔽
	UART0->UARTIMSC_b.Transmit_IM =1; // 关闭发送中断屏蔽

	UART0->UARTDMACR_b.TxFIFO_en = 1; // TxFIFO的DMA使能，高电平有效。
...
{
	if(Uart.IntSendFlag) return;
	
	if(Uart.UnSendCnt > 0)
	{
		if(Uart.IntSendFlag==0) Uart.UnSendCnt--;			// 如不减一，结果会多发一个字节
...
void UART0_IRQHandler(void)
{
	...
	if(temp32&(1<<5))
	{
		...
		if(Uart.UnSendCnt > 0)
		{
			...
			if(Uart.SBReadIdx >= UART_SEND_BUF_SIZE)
			{
				Uart.SBReadIdx = 0;
			}
		}
	...
	if (temp32&(1<<4))
	{
		Uart.RevBuf[Uart.RBWriteIdx] = UART0->UARTDR;
